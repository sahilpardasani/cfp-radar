...
export async function chat(
  ...
  if (!res.ok) {
    let upstreamMessage = "";
    try {
      const raw = await readResponseText(res, 64 * 1024);
      upstreamMessage = JSON.parse(raw)?.error?.message || "";
    } catch {
      // The status, provider and model below are enough to diagnose most
      // failures; never dump an arbitrary upstream body into production logs.
    }
    console.error("[llm] upstream request failed", {
      provider: cfg.provider,
      model: cfg.model,
      status: res.status,
      requestId: res.headers.get("x-request-id") || res.headers.get("x-groq-request-id") || undefined,
      message: String(upstreamMessage).replace(/\s+/g, " ").slice(0, 300) || undefined,
  ...
  console.info("[llm] completion", {
    provider: cfg.provider,
    model: cfg.model,
    elapsedMs: Date.now() - startedAt,
    promptTokens: data?.usage?.prompt_tokens,
    completionTokens: data?.usage?.completion_tokens,
    queueSeconds: data?.usage?.queue_time,
    promptSeconds: data?.usage?.prompt_time,
    completionSeconds: data?.usage?.completion_time,
  });
